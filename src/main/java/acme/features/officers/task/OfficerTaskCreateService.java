...
public class OfficerTaskCreateService implements AbstractCreateService<Officer, Task> {
	...
	public void validate(final Request<Task> request, final Task entity, final Errors errors) {
		...
		if (!errors.hasErrors("end") && !errors.hasErrors("start")) {
			errors.state(request, entity.getEnd().after(entity.getStart()), "end", "officer.task.error.end");
		}
		
		if (!errors.hasErrors("start")) {
			Date moment;
	        moment = new Date(System.currentTimeMillis() - 1);
			errors.state(request, entity.getStart().after(moment), "start", "officer.task.error.start");
		}
		...
		if (!errors.hasErrors("description")) {
			errors.state(request, Filter.filterString(entity.getDescription(),this.personalizationRepository.findCensoredWords(), this.thresholdRepository.findThresholdById()), "description", "officer.task.form.error.description");
		}

		if (!errors.hasErrors("title")) {
			errors.state(request, Filter.filterString(entity.getTitle(),this.personalizationRepository.findCensoredWords(), this.thresholdRepository.findThresholdById()), "title", "officer.task.form.error.title");
		}
